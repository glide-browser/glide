...
export function pretty(glide: Glide, browser: Browser): void {
  browser.webNavigation.onCompleted.addListener(async details => {
    await glide.content.execute(() => {
      ...
      const button = document!.querySelector("#js-download-hero") as HTMLElement | null;
      ...
      const dodge = (e: MouseEvent) => {
        ...
        let offset_x = Math.cos(0.2 * (Math.random() - 0.5) + angle) * MOVE_STEP_PX;
        let offset_y = Math.sin(0.2 * (Math.random() - 0.5) + angle) * MOVE_STEP_PX;
