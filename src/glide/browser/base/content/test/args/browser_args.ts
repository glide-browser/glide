...
const { parse_command_args } = ChromeUtils.importESModule("chrome://glide/content/utils/args.mjs");
...
add_task(async function test_space_separated_args() {
  const parsed = parse_command_args({
    ...
    schema: { "--mode": { type: "string", required: true }, "--int": { type: "integer", required: true } },
  });
...
add_task(async function test_boolean_flag_without_value() {
  const parsed = parse_command_args({
    ...
    schema: { "--verbose": { type: "boolean", required: false }, "--quiet": { type: "boolean", required: false } },
  });
...
add_task(async function test_positional_arguments() {
  const parsed = parse_command_args({
    ...
    schema: {
      input: { type: "string", required: true, position: 0 },
      output: { type: "string", required: true, position: 1 },
...
add_task(async function test_extra_arguments() {
  ...
  is(parsed.args["--mode"], "normal");
  is(JSON.stringify(parsed.remaining), JSON.stringify(["extra1", "extra2"]), "should capture remaining args");
});

add_task(async function test_invalid_value_for_required_flag() {
  const parsed = parse_command_args({
    args: "--count=invalid --mode=normal",
    schema: { "--count": { type: "integer", required: true }, "--mode": { type: "string", required: true } },
  });
  ok(!parsed.valid, "should be invalid with bad value");
  is(parsed.errors.length, 1);
  ok(parsed.errors.some(e => e.includes("not a valid integer")), "should have integer validation error");
  ok(
...
add_task(async function test_invalid_value_for_required_positional_flag() {
  const parsed = parse_command_args({
    args: "invalid",
    schema: { count: { type: "integer", required: true, position: 0 } },
  });
  ok(!parsed.valid, "should be invalid with bad value");
  is(parsed.errors.length, 1);
  ok(parsed.errors.some(e => e.includes("not a valid integer")), "should have integer validation error");
  ok(
    !parsed.errors.some(e => e.includes("Required positional argument \"count\" is missing")),
    "should not have missing flag error when invalid value provided",
  );
});
...
add_task(async function test_multiple_spaces_and_trimming() {
  const parsed = parse_command_args({
    ...
    schema: { "--mode": { type: "string", required: true }, "--verbose": { type: "boolean", required: false } },
  });
...
add_task(async function test_boolean_values() {
  ...
  for (const { input, expected } of test_cases) {
    const parsed = parse_command_args({
      args: `--flag=${input}`,
      schema: { "--flag": { type: "boolean", required: true } },
    });
...
add_task(async function test_enum_values() {
  ...
  for (const { input, expected } of valid_cases) {
    const parsed = parse_command_args({
      args: `--flag=${input}`,
      schema: { "--flag": { type: { enum: ["a", "b", "c"] }, required: true } },
    });
...
add_task(async function test_missing_value_for_flag() {
  const parsed = parse_command_args({ args: "--mode", schema: { "--mode": { type: "string", required: true } } });
  ok(!parsed.valid, "should be invalid with missing value");
  ok(parsed.errors.some(e => e.includes("No value provided")), "should have missing value error");
});
...
add_task(async function test_missing_positional_argument() {
  ...
  ok(
    parsed.errors.some(e => e.includes("Required positional argument \"output\" is missing")),
    "should have missing positional error",
  );
});

add_task(async function test_quoted_strings_with_spaces() {
  const parsed = parse_command_args({ args: "foo \"bar baz\"", schema: {} });
...
add_task(async function test_quoted_strings_in_flags() {
  const parsed = parse_command_args({
    ...
    schema: { "--message": { type: "string", required: true }, "--path": { type: "string", required: true } },
  });
...
add_task(async function test_mixed_quotes() {
  const parsed = parse_command_args({
    ...
    schema: { "--message": { type: "string", required: true }, "--other": { type: "string", required: true } },
  });
...
add_task(async function test_empty_quoted_strings() {
  const parsed = parse_command_args({
    args: "--message=\"\" --path=\"\"",
    schema: { "--message": { type: "string", required: true }, "--path": { type: "string", required: false } },
  });
